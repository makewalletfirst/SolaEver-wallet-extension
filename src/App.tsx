...
interface TokenWithBalance {
  mint: string; symbol: string; name: string; balance: number; image?: string;
}

type CustomTokenInfo = { symbol: string; name: string; image?: string };

const METADATA_PROGRAM_ID = new PublicKey('metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s');

/**
 * Metaplex Token Metadata PDA 에서 name/symbol/uri 를 정확히 읽어서
 * off-chain JSON 의 `image` 까지 가져온다. URI 4-byte length prefix 를 제대로
 * skip 하기 위해 readUInt32LE 로 변동 길이 파싱 (이전 fixed-slice 방식은 0xC8
 * prefix 가 URL 앞에 붙어 fetch 실패).
 */
async function fetchTokenMetadata(mintPubkey: PublicKey): Promise<{ name: string; symbol: string; image?: string } | null> {
  ...
  // header: 1 (key) + 32 (update_authority) + 32 (mint) = 65
  let off = 65;
  const readStr = () => {
    const len = data.readUInt32LE(off); off += 4;
    const s = data.slice(off, off + len).toString('utf-8').replace(/\0/g, '').trim();
    off += len;
    return s;
  };
  let name = '', symbol = '', uri = '';
  try {
    ...
    symbol = readStr();
    uri = readStr();
  } catch { /* malformed; fall through with whatever we got */ }

  let image: string | undefined;
  if (uri && /^https?:\/\//i.test(uri)) {
    try {
      const j = await fetch(uri, { cache: 'no-store' }).then(r => r.ok ? r.json() : null);
      if (j && typeof j.image === 'string' && j.image.length > 0) image = j.image;
    } catch {/* off-chain unreachable; OK */}
  }
...
export default function App() {
  ...
  const [customTokens, setCustomTokens] = useState<Record<string, CustomTokenInfo>>({});
  ...
  const [tokenImageInput, setTokenImageInput] = useState('');
  ...
  async function init() {
    ...
    chrome.storage.local.get(['solaever_custom_tokens'], async (res) => {
       if (res.solaever_custom_tokens) {
         const stored = res.solaever_custom_tokens as Record<string, CustomTokenInfo>;
         setCustomTokens(stored);
         // ── 자동 마이그레이션: 이전 버전에서 image 없이 저장된 토큰을 1회 재조회 ──
         //   사용자가 LNsola 같은 토큰을 이미지 없이 추가한 상태여도 한 번만 재페치해
         //   image 를 채워 storage 에 다시 저장.
         const missing = Object.entries(stored).filter(([, v]) => !v.image);
         if (missing.length > 0) {
           const updated: Record<string, CustomTokenInfo> = { ...stored };
           let changed = false;
           for (const [mint, info] of missing) {
             try {
               const meta = await fetchTokenMetadata(new PublicKey(mint));
               if (meta?.image) {
                 updated[mint] = { ...info, image: meta.image };
                 changed = true;
               }
             } catch { /* skip */ }
           }
           if (changed) {
             chrome.storage.local.set({ solaever_custom_tokens: updated });
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    // ── 송금 전 지갑 비밀번호 검증 ──
    if (!sendPassword) {
      setStatus({ type: 'error', message: 'Enter wallet password.' });
      return;
    }
    if (sendPassword !== currentWallet.password) {
    ...
    try {
      ...
      // sendSLE / sendSPLToken 내부에서 잔액 부족·on-chain 실패 시 throw
      let sig = selectedAsset === 'SLE'
        ? await sendSLE(kp, recipient, parsed)
        : await sendSPLToken(kp, selectedAsset, recipient, parsed);
      setStatus({ type: 'success', message: 'Transaction Sent Successfully!' });
      setRecipient(''); setAmount(''); setSendPassword('');
  ...
  const handleCheckToken = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const meta = await fetchTokenMetadata(mintPubkey);
      if (meta) {
        setTokenNameInput(meta.name || "Custom Token");
        setTokenSymbolInput(meta.symbol || "TOKEN");
        if (meta.image) setTokenImageInput(meta.image);
      } else {
  ...
  const handleSaveToken = (e?: React.FormEvent) => {
    if (e) e.preventDefault();
    const entry: CustomTokenInfo = { symbol: tokenSymbolInput, name: tokenNameInput };
    if (tokenImageInput) entry.image = tokenImageInput;
    const map = { ...customTokens, [addTokenAddress.trim()]: entry };
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-900 pb-20">
            ...
            <div className="space-y-2 px-1 pb-10">
              ...
              <div className="bg-slate-800/40 p-4 rounded-2xl flex items-center justify-between cursor-pointer group hover:bg-slate-800/60 transition-colors" onClick={() => setView('receive')}>
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl overflow-hidden flex items-center justify-center bg-slate-800/40">
                    <img src="/SLEicon.png" alt="SLE" className="w-full h-full object-contain" />
                  </div>
                  <div><div className="font-bold text-sm">SolaEver</div><div className="text-[10px] text-slate-500">Native Coin</div></div>
                </div>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-slate-700/50 rounded-xl flex items-center justify-center overflow-hidden">
                      {info.image
                        ...
                        : <span className="font-bold text-blue-400 text-sm">{info.symbol[0]}</span>}
                    </div>
                    <div><div className="font-bold text-sm">{info.name}</div><div className="text-[10px] text-slate-500">{info.symbol}</div></div>
                  </div>
        ...
        {view === 'send' && currentWallet && (
           <form onSubmit={(e)=>{e.preventDefault();handleSend();}} className="p-6 space-y-6 animate-in slide-in-from-right-4">
             <div className="flex items-center gap-2"><button type="button" onClick={() => { setSendPassword(''); setView('home'); }} className="p-2 hover:bg-slate-800 rounded-full text-slate-400"><X size={20} /></button><h2 className="text-xl font-bold">Send</h2></div>
             <div className="space-y-4">
                ...
                <input type="password" value={sendPassword} onChange={(e) => setSendPassword(e.target.value)} placeholder="Wallet Password" className="w-full bg-slate-800 border border-slate-700 rounded-xl p-4 text-sm outline-none focus:ring-2 focus:ring-blue-500" autoComplete="current-password" />
             </div>
                ...
                <form onSubmit={(e)=>{e.preventDefault();handleSaveToken();}} className="space-y-4 p-4 bg-slate-950 rounded-3xl border border-slate-800 animate-in zoom-in-95">
                   {tokenImageInput && (
                     <div className="flex items-center gap-3 px-1">
                       <img src={tokenImageInput} alt={tokenSymbolInput} className="w-10 h-10 rounded-xl object-cover bg-slate-800" />
                       <span className="text-[10px] text-slate-500">Logo auto-fetched from Metaplex URI</span>
                     </div>
                   )}
        ...
        {view === 'token-detail' && selectedToken && (
           <div className="p-6 space-y-6 animate-in zoom-in-95">
             ...
             <div className="flex flex-col items-center py-8 bg-slate-800/30 rounded-3xl border border-slate-700/50">
                <div className="w-16 h-16 rounded-2xl overflow-hidden flex items-center justify-center bg-slate-800/40 mb-4">
                  {selectedToken.image
                    ? <img src={selectedToken.image} alt={selectedToken.symbol} className="w-full h-full object-contain" />
                    : <span className="text-2xl font-black text-blue-500">{selectedToken.symbol[0]}</span>}
                </div>
                ...
                return (
                  <div key={i} className="bg-slate-800/30 p-4 rounded-2xl flex items-center justify-between border border-transparent hover:border-slate-700 group transition-all">
                     <div className="flex items-center gap-3 min-w-0">
                       <div className={`w-2 h-2 rounded-full shrink-0 ${failed ? 'bg-red-500' : 'bg-emerald-500'}`}></div>
                       <div className="min-w-0">
                         <div className="font-bold text-xs truncate">{failed ? 'Failed' : 'Success'} · {tx.signature.slice(0, 12)}...</div>
                         <div className="text-[9px] text-slate-500">{tx.blockTime ? new Date(tx.blockTime * 1000).toLocaleString() : 'pending'}</div>
                       </div>
                     </div>
                     <a href={`${EXPLORER_URL}/tx/${tx.signature}`} target="_blank" rel="noreferrer" className="p-2 text-slate-500 hover:text-white transition-colors shrink-0"><ExternalLink size={14} /></a>
                  </div>
                );
              })}
           </div>
